 ...
 */
inline unsigned int LDSketch_find(LDSketch_t* tbl, data_type key, int row_no) {
  unsigned int oper;
  ...
  */
  ...
  ret_bucket = hash32(key, oper) % (tbl->w);
 ...
 */
void LDSketch_update(LDSketch_t* sk, data_type key, long long val) {
  int j, k;
...
long long LDSketch_low_estimate(LDSketch_t* sk, data_type key) {
  long long ret = 0;
  for (int i = 0; i < sk->h; ++i) {
    int k = LDSketch_find(sk, key, i);
    int index = i * sk->w + k;
...
long long LDSketch_up_estimate(LDSketch_t* sk, data_type key) {
  int k = LDSketch_find(sk, key, 0);
  long long ret = dyn_tbl_up_estimate(sk->tbl[k], key);
  for (int i = 1; i < sk->h; ++i) {
    k = LDSketch_find(sk, key, i);
    int index = i * sk->w + k;
...
/*
unsigned int LDSketch_find_cand(LDSketch_t* sk, LDSketch_t* sk_old, double
thresh, unsigned char** cand_list, unsigned int MAX_CAND) {
