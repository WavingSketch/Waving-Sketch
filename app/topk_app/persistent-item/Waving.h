 ...
 public:
  struct Bucket {
    ...
    void Insert(const data_type item) {
      uint32_t choice = hash32(item, 17) & 1;
      ...
      if (incast * COUNT[choice] >= int(min_num * factor)) {
        if (real[min_pos]) {
          uint32_t min_choice = hash32(items[min_pos], 17) & 1;
  ...
  Count_Bucket(uint32_t _MEM, int _HIT, uint32_t _HASH_NUM = 3)
      : BUCKET_NUM(_MEM /
                   (slot_num * (sizeof(data_type) + sizeof(count_type) + 1) +
                    sizeof(count_type)) *
                   1 / 2),
        HASH_NUM(_HASH_NUM),
        LENGTH(_MEM * 8 * 1 / 2) {
  ...
  void Init(const Data& data, uint time) {
    ...
    if (!init) {
      uint32_t bucket_pos = hash32(data) % BUCKET_NUM;
      buckets[bucket_pos].Insert(data);
    }
  }

  int Query(const data_type& item) {
    uint32_t bucket_pos = hash32(item) % BUCKET_NUM;
