...
void dyn_tbl_get_heavy_key(dyn_tbl_t* dyn_tbl, long long thresh,
                           data_type* keys, int* num_key) {
  long long bias = dyn_tbl->decrement;
  thresh = thresh - bias;
  // for(std::unordered_map<dyn_tbl_key_t, long long>::iterator it =
  // dyn_tbl->array.begin(); it != dyn_tbl->array.end(); ++it) {
  int n = 0;
...
//       char addr2[30];
//       unsigned int* ptr = (unsigned int*)key.key;
//       fprintf(fp, "%s %s %lld\n", ip2a(*ptr, addr1), ip2a(*(ptr + 1), addr2),
//               it->second);
//     }
//   }
...
void dyn_tbl_update(dyn_tbl_t* dyn_tbl, data_type key, int val) {
  ...
  if (value > dyn_tbl->max_value) dyn_tbl->max_value = value;
}
...
void dyn_tbl_destroy(dyn_tbl_t* dyn_tbl) { delete dyn_tbl; }
