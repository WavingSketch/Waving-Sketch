...
#define MAX_KEYLEN sizeof(data_type)

 /**
  * Structure of key
  */
typedef data_type dyn_tbl_key_t;

/**
 * Object for hash
 */
struct dyn_tbl_key_hash {
	/// overloaded operation
	long operator()(const dyn_tbl_key_t& k) const {
		return hash32(k, 17);
	}
};
...
// typedef std::unordered_set<dyn_tbl_key_t, dyn_tbl_key_hash, dyn_tbl_key_eq>
// myset;
...
typedef struct dyn_tbl_s {
	...
	std::unordered_map<dyn_tbl_key_t, long long, dyn_tbl_key_hash>
		array;

	/// total sum: V(i,j)
	long long total;

	/// maximum length of counters allowed, exceeding this value would trigger
	/// expansion: l(i, j)
...
void dyn_tbl_get_heavy_key(dyn_tbl_t* dyn_tbl, long long thresh,
	unsigned char* keys, int* num_key);

/// estimate the lower sum of a key
// @param dyn_tbl target bucket
// @param key
// @return the estimated lower sum
long long dyn_tbl_low_estimate(dyn_tbl_t* dyn_tbl, data_type key);

/// estimate the higher sum of a ket
// @param dyn_tbl target bucket
// @param key
// @return the estimated higher sum
long long dyn_tbl_up_estimate(dyn_tbl_t* dyn_tbl, data_type key);
 ...
 ************************************************************/
  ...
  */
void dyn_tbl_update(dyn_tbl_t* dyn_tbl, data_type key, int val);
